...
st.set_page_config(page_title="Аналитика Nуч (Модуль 1)", layout="wide")
...
def load_admin_structure(url):
    try:
        ...
        # Очистка заголовков: убираем пробелы и в верхний регистр
...
if uploaded_file and df_struct is not None:
    try:
        ...
        # Нормализация колонок входного файла
        df_raw.columns = [str(col).strip().upper() for col in df_raw.columns]
        
        # Интеллектуальный поиск колонки КМ (варианты: КМ, KM, КМ., КИЛОМЕТР)
        km_aliases = ["КМ", "KM", "КМ.", "КИЛОМЕТР", "№ КМ"]
        actual_km_col = next((c for c in df_raw.columns if c in km_aliases), "КМ")
        
        # Если колонки нет, создаем пустую, чтобы код не упал, но уведомляем
        if actual_km_col not in df_raw.columns:
            st.warning(f"Колонка КМ не найдена. Ожидались: {km_aliases}")
            df_raw[actual_km_col] = 0
        ...
        with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
            ...
            # Список колонок для вкладок (с учетом найденного КМ)
            cols_to_extract = ["КОДНАПР", "ПУТЬ", "ПД", actual_km_col, "ОЦЕНКА", "ПРИЧИНА"]
            
            for score, s_name in {5: "Отличные", 4: "Хорошие", 3: "Удовл", 2: "Неуд"}.items():
                subset = df_eval[df_eval["ОЦЕНКА"] == score]
                # Берем только существующие в данных колонки из нашего списка
                final_cols = [c for c in cols_to_extract if c in subset.columns]
                subset[final_cols].to_excel(writer, sheet_name=s_name, index=False)
        ...
        st.download_button(
            ...
            file_name=f"Nuch_Report_{datetime.now().strftime('%d_%m_%Y')}.xlsx",
        ...
        for i, score in enumerate([5, 4, 3, 2]):
            with tabs[i+1]:
                subset = df_eval[df_eval["ОЦЕНКА"] == score]
                cols_to_show = [c for c in cols_to_extract if c in subset.columns]
                if not subset.empty:
